	...
	*/
	...
	for(let i = 0; i < results.data.web.results.length; i++){
		...
		if(affiliatesEnabled && typeof(affiliates[url]) !== 'undefined'){
			...
			html += `<a href="${escapeHtml(url)}" class="primaryColor text-lg"><img src="${favicon}" loading="lazy" width="16" height="16" alt="🌐" /> ${escapeHtml(results.data.web.results[i].title)}</a>`;
		}
