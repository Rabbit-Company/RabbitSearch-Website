...
function displayVideoResults(results){
	...
	for(let i = 0; i < results.data.items.length; i++){
		...
		if(typeof(results.data.items[i].thumbnails.high) === 'undefined') continue;
		...
		html += `
			<div class="group aspect-w-10 aspect-h-7 block w-full overflow-hidden rounded-lg bg-gray-100 focus:outline-none">
				<a href="${url}">
					<img src="${escapeHtml(results.data.items[i].thumbnails.high)}" alt="${name}" loading="lazy" class="object-cover group-hover:opacity-75">
				</a>
			</div>
