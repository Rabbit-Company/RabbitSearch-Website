...
function displayVideoResults(results){
	...
	let totalEstimatedMatches = results.data.pageInfo?.totalResults || 0;
	html += `<p class="secondaryColor text-sm">About ${totalEstimatedMatches.toLocaleString()} results (${querySpeed}ms)</p>`;
	...
	for(let i = 0; i < results.data.items.length; i++){
		...
		const url = "https://www.youtube.com/watch?v=" + results.data.items[i].id.videoId;

		html += `<li class="relative">`;
		html += `
			<div class="group aspect-w-10 aspect-h-7 block w-full overflow-hidden rounded-lg bg-gray-100 focus:outline-none">
				<a href="${url}">
					<img src="${escapeHtml(results.data.items[i].snippet.thumbnails.high.url)}" alt="${name}" loading="lazy" class="object-cover group-hover:opacity-75">
				</a>
			</div>
			...
			<p class="secondaryColor block text-sm font-medium truncate">Youtube &middot; <a href="https://www.youtube.com/channel/${ results.data.items[i].snippet.channelId}">${escapeHtml(results.data.items[i].snippet.channelTitle)}</a></p>
		`;
		html += "</li>";
	}
	html += "</ul>";
